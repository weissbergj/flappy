...
class TinyTransformerLM(nn.Module):
    def __init__(self, vocab_size: int, seq_len: int, d_model=512, n_layers=8, n_heads=8, causal=True):
        ...
        # RoPE cache (cos/sin) with shape (1,1,seq_len+1,d_head) so position_ids in [0,seq_len] work (permute AR)
    ...
    def forward(self, idx, position_ids=None):
        ...
        if position_ids is None:
            cos = self.rope_cos[:, :, :T, :].to(dtype=x.dtype)  # (1,1,T,D)
            sin = self.rope_sin[:, :, :T, :].to(dtype=x.dtype)
        else:
            # position_ids: (B,T) longs in [0, seq_len]
            assert position_ids.max().item() <= self.seq_len
            # gather -> (B,T,D) then unsqueeze to (B,1,T,D) for broadcasting over heads
            cos = self.rope_cos[0, 0, position_ids, :].to(dtype=x.dtype).unsqueeze(1)
            sin = self.rope_sin[0, 0, position_ids, :].to(dtype=x.dtype).unsqueeze(1)
