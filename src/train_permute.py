...
from src.model_permute import TinyTransformerLM
...
def make_orders(L: int, N: int, seed: int):
    # L here is length of tokens[1:], i.e. seq_len
    g = torch.Generator(device="cpu")
    g.manual_seed(seed)

    base = torch.arange(L, dtype=torch.long)
    orders = [base]  # O[0] = identity

    for i in range(1, N):
        # "graded" noise like paper (optional); this is faithful + simple
        sigma = float(i)
        noise = torch.randn(L, generator=g) * sigma
        scores = base.float() + noise
        pi = torch.argsort(scores)  # permutation of 0..L-1
        orders.append(pi.to(torch.long))

    return torch.stack(orders, dim=0)  # (N, L)
...
def main():
    ...
    ap.add_argument("--num_orders", type=int, default=1, help="AR only: number of fixed token orderings (N)")
    ap.add_argument("--orders_seed", type=int, default=0, help="seed for generating fixed permutations")
    args = ap.parse_args()
    ...
    if is_main:
        ...
        wandb.init(
            ...
            name=f"{args.mode}_N{args.num_orders}_tok{args.max_tokens}_steps{args.steps}",
            config=vars(args),
        )
    ...
    orders = None
    if args.mode == "ar" and args.num_orders > 1:
        orders = make_orders(args.seq_len, args.num_orders, args.orders_seed)  # (N, L)
        orders = orders.to(device)

    def permute_ar_batch(x, y, orders, device):
        """
        x,y: (B,L) from dataset (L = seq_len)
        Reconstruct tokens length L+1, then shuffle tokens[:,1:] using a per-example sampled π.
        Returns:
          x_in: (B,L) input tokens (permuted)
          y_tgt: (B,L) target tokens (permuted shift)
          pos_ids: (B,L) RoPE position ids with token0 anchored at 0
        """
        x = x.to(device, non_blocking=True)
        y = y.to(device, non_blocking=True)

        B, L = x.shape  # L = seq_len
        # reconstruct tokens of length L+1 (need last label token)
        tokens = torch.cat([x, y[:, -1:].contiguous()], dim=1)  # (B, L+1)

        # sample an ordering index per example
        idx = torch.randint(low=0, high=orders.size(0), size=(B,), device=device)
        pi = orders.index_select(0, idx)  # (B, L) each row is a permutation of [0..L-1]

        # shuffle tokens except first token (length L segment)
        shuffled = tokens.clone()
        # gather per-example: tokens[:,1:] shape (B,L), pi shape (B,L)
        shuffled[:, 1:] = torch.gather(tokens[:, 1:], dim=1, index=pi)

        # standard AR shift on permuted sequence
        x_in = shuffled[:, :-1]   # (B,L)
        y_tgt = shuffled[:, 1:]   # (B,L)

        # RoPE position ids: anchor first token at pos 0, assign 1..L-1 using pi[:, :L-1]
        pos = torch.empty((B, L), device=device, dtype=torch.long)
        pos[:, 0] = 0
        pos[:, 1:] = pi[:, : L - 1] + 1  # (B,L-1), in [1..L]

        return x_in, y_tgt, pos
        ...
        with torch.autocast(device_type="cuda", dtype=autocast_dtype, enabled=use_amp):
            if args.mode == "ar":
                if orders is not None:
                    x_in, y_tgt, pos_ids = permute_ar_batch(x, y, orders, device)
                    if is_main and step % 500 == 0:
                        assert pos_ids.max() <= args.seq_len  # allow == (max pos is L = seq_len)
                        assert (x_in[:, 0] == x.to(device)[:, 0]).all()
                    logits = model(x_in, position_ids=pos_ids)
                    loss = ar_loss(logits, y_tgt)
                else:
                    x = x.to(device, non_blocking=True)
                    y = y.to(device, non_blocking=True)
                    logits = model(x)
